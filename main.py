...
def main_parse(dict):
    ...
    describe = dict.get('describe')
    details_data = format_base_spdb.get_true(url=new_url)
    dict['true_url'] = new_url
    new_keyword = dict.get('keyword')
    number = str(uuid.uuid1()).replace('-', '')
    dict['number'] = number
    format_base_spdb.get_(new_keyword=new_keyword, new_tittle=new_tittle, details_data=details_data, true_url=new_url,
                          dict=dict, number=number,describe=describe)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            flag_data = conn.search_all_data(redis_key='qingting')
            ...
            if flags == 0:
                ...
                # 实例化sql链接
                dta = conn.search_data_redis(redis_key='qingting')
                dict = json.loads(dta)
